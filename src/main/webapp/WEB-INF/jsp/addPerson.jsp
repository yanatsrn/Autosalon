...
<body>
<c:url var="add_person" value="/addPerson"/>
<div class="wrapper">
    <div class="conteiner">
        <div class="window-users">
            <form:form action="${add_person}" method="post" modelAttribute="person" class="transparent">
                <div class="title title-reg">Регистрация</div>
                <!-- фамилия и имя -->
                <div class="forms">
                    <div class="field">
                        <p>Фамилия</p>
                    ...
                    <div class="field">
                        <div class="discount">
                            <form:input value="0" hidden="hidden" path="user.discountSum"/>
                        </div>
                    </div>
                        <c:if test="${addPersonSuccess}">
                            <div >Пользователь ${savedPerson.surname} ${savedPerson.name} добавлен!</div>
                        </c:if>
                        <c:if test="${addPersonSuccess == false}">
                            <div class="mistake">
                                <p>${personError}</p>
                            </div>
                        </c:if>
                    <!-- кнопки зарегестрироваться и отмена -->
                    <div class="btns">
                        <input type="submit" value="Зарегистрироваться" class="btn btn-reg">
                        <a href="/index.jsp"><input type="button" value="Отмена" class="btn btn-reg"></a>
                    </div>
                </div>
            </form:form>
        </div>
